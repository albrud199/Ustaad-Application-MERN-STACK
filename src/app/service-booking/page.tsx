...
export default function ServiceBookingPage() {
  ...
  const garage = searchParams.get("garage") || "Selected garage";
      ...
      <main className="flex-1 pt-32 pb-24 px-6 max-w-4xl mx-auto w-full relative z-10">
        <div className="glass-card rounded-3xl p-8 md:p-12 border border-outline-variant/15 space-y-8">
          <div>
            ...
            <h1 className="text-4xl font-extrabold font-[family-name:var(--font-headline)] text-on-surface">{garage}</h1>
            <p className="text-on-surface-variant mt-2">Selected garage rating {rating} and pricing tier {pricing}.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="rounded-2xl border border-outline-variant/15 p-5 bg-surface-container-low"><p className="text-xs uppercase tracking-widest text-on-surface-variant">Step 1</p><p className="font-bold mt-1">Confirm garage</p></div>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <Link href="/service-results" className="px-6 py-4 rounded-xl border border-outline-variant/20 text-on-surface font-bold hover:bg-surface-container-high transition-colors text-center">Back to garages</Link>
          </div>
        </div>
      </main>
